...
const StaticCard = ({
	...
	return (
		<CardContainer onClick={onClick} className="static-card" ref={ref} $isCompleted={isCompleted}
			...
			<CardHeaderStyle>
				...
				<div className="star-static-card" onClick={onHandleUpdateStatusTask}>{isChallenge ? <Cup/> : <Star/>}
				</div>
			</CardHeaderStyle>
			<div className="main-card">
				<h5>{isChallenge && ("CHALLENGE")}</h5>
				<h3 className="title-task">{textPropName?.length >= 18 ? `${textPropName.slice(0, 18)}...` : textPropName}</h3>
				<div>
					<span className="date">{isChallenge && "by "}{time}</span>
					{isFireShow && <Fire/>}
				</div>
			</div>
			<CategoryContainer className={categoryProp}>{categoryProp}</CategoryContainer>
			{isCompleted && <Button onClick={onRecoverTask}
									className="recover-task-btn">Recover {isChallenge ? 'challenge' : 'task'}<Arrow/></Button>}
		</CardContainer>
	)
}
